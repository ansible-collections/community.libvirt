...
Architecture and check_mode categories are fully covered by `AGENTS.md` — apply those sections directly.
...
- Module file names follow the collection's naming conventions.
...
- No sensitive data appears in module return values in plaintext.
...
- Integration test pattern is followed:
  ...
  3. Verify actual state independently → `register: result` → `ansible.builtin.assert`
...
- No security vulnerabilities: no shell injection, no unvalidated external input, no hardcoded credentials.
...
Use `N/A` for categories that do not apply to the changeset (e.g., testing for a docs-only PR). Be specific: always reference the file and line number when citing a finding.
